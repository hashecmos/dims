...
@Component({
  selector: 'documentlaunch-info',
  templateUrl: 'app/SharedComponents/documentLaunchinfo.component.html'
})

export class DocumentLaunchInfo{
  ...
  //private myDatePickerOptions: IMyOptions = global.date_picker_disabled;
  ...
  ngOnChanges() {
    ...
    // console.log("this.selectedFirstTab  ::: ",this.selectedFirstTab)
    if(this.currentTab == "")
        this.currentTab =this.selectedFirstTab;
    else
        this.selectedFirstTab = this.currentTab;
    // console.log("this.currentTab AFTER  :: ",this.currentTab)
    // console.log("this.selectedFirstTab AFTER ::: ",this.selectedFirstTab)
    ...
    modifyDocProperties($event: any){
        ...
        for (let inputField of inputTagList) {
            ...
            for( let prop of this.attachmentProperties){
             ...
             if(this.selectedDocument[0] && this.selectedDocument[0].referenceNo ){
                            this.selectedDocument[0].referenceNo = inputField.value;
                   ...
                   /* if(inputField.id =='DocumentTitle'){
                    ...
                    if(inputField.id =='EmailSubject'){
                     ...
                     if(this.selectedDocument[0] && this.selectedDocument[0].subject ){
                         this.selectedDocument[0].subject = inputField.value;
                         this.selectedDocument[0].EmailSubject = inputField.value;
                         this.docChanged =true;
                     }
        ...
        for( let prop of this.attachmentProperties){
            if(prop.propertyName =='DocumentTitle'){
                 ...
                 if(this.selectedDocument[0] && this.selectedDocument[0].DocumentTitle ){
                    this.selectedDocument[0].symbolicName = EmailSub;
                   }
                   this.docChanged =true;
             }
             if(prop.propertyName == 'DocumentDate'){
        ...
        for (let selectField of selectTagList) {
         ...
         //SUBJECT, Document Date, Date Recived , Document To
         //console.log("updated properties :: ", JSON.parse(JSON.stringify(this.attachmentProperties)));
         //console.log("document id :: "+ this.selectedAttachmentID);
         //console.log("objectStore name :: "+ global.os_name);
         ...
         this.workflowService.updateDocumentProperties(JSON.stringify(submissionValues)).subscribe(_res => {
            if( this.docChanged ){
                this.docAfterChange.emit(this.selectedDocument[0]); 
            }
         });
