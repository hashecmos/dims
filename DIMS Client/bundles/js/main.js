  ...
  return {
    ...
    init: function (options) {
      ...
      //Scroller plugin init
        ...
        var remainingH = $(window).height()-topNav;
        $(".page-aside").height(remainingH-1);
        $(".be-content").height(remainingH-1);

        var pagination = $('.pagination');
        pagination.hide();

        $('.pg-prv').on('click', function(){
          $('.pagination-previous>a')[0].click();
        }); 

        $('.pg-nxt').on('click', function(){
        ...
        $('.email-filters').on('click','.dropdown-toggle',function(){
...
function setHeight() {
    var topNav = $("header").outerHeight();
    var topMisc = $("#reportSearchForm").outerHeight() + $("#reportBar").outerHeight();
    var remainingH = $(window).height() - topNav - topMisc;
    $("#printSectionId").height(remainingH - 34);
}

$(document).ready(function() {
  $('my-app').on('click','#reportSearchForm>.toggle', function() {
    $(this).parent().toggleClass('closedForm',200);
    setTimeout(function(){ setHeight(); }, 205);
  });
})
